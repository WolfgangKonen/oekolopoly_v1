...
class Camera(pygame.sprite.Group):
    ...
    def custom_draw(self):
        ...
        self.can_use_buttons = self.zoom_factor == self.min_zoom
...
class Game:
    def __init__(self, camera):
        ...
        # region labels
        Label(Vector2(390, 48), Vector2(155, 40), camera, "Aktionspunkte", 23, color_white)
        Label(Vector2(815, 40), Vector2(115, 40), camera, "Sanierung", 23, color_white)
        Label(Vector2(1250, 30), Vector2(122, 40), camera, "Produktion", 23, color_white)
        Label(Vector2(1725, 380), Vector2(180, 40), camera, "Umweltbelastung", 23, color_white)
        Label(Vector2(1790, 830), Vector2(125, 40), camera, "Aufklärung", 23, color_white)
        Label(Vector2(1025, 980), Vector2(155, 40), camera, "Lebensqualität", 23, color_white)
        Label(Vector2(615, 980), Vector2(180, 40), camera, "Vermehrungsrate", 23, color_white)
        Label(Vector2(215, 980), Vector2(140, 40), camera, "Bevölkerung", 23, color_white)
        Label(Vector2(15, 435), Vector2(78, 40), camera, "Politik", 23, color_white)
    ...
    def check_button_press(self):
        if not self.help_screen.active:
            if not self.done:
                ...
                if not self.predict_used and self.predict_usages > 0 and self.predict_button.button_pressed():
...
class Button(pygame.sprite.Sprite):
    def __init__(self, pos, size, color, camera, text, font_size=30):
        ...
        self.size = Vector2(size.x / 1920 * pygame.display.get_window_size()[0], size.y / 1080 * pygame.display.get_window_size()[1])
        self.image = pygame.Surface(self.size)
        self.image.fill(color)
        self.color = color
        self.camera = camera
        self.rect = self.image.get_rect()
        self.rect.topleft = (pos.x / 1920 * pygame.display.get_window_size()[0], pos.y / 1080 * pygame.display.get_window_size()[1])
    ...
    def draw(self):
        ...
        self.image.blit(self.button_text_font.render(f"{self.text}", True, color_black), (10 / 1920 * pygame.display.get_window_size()[0], text_position))
    ...
    def button_pressed(self) -> bool:
        ...
        button_rect.topleft = self.rect.topleft + self.camera.offset
...
class Diagram(pygame.sprite.Sprite):
    def __init__(self, pos, camera, min_value, max_value):
        super().__init__(camera)
        self.size = Vector2(35 / 1920 * pygame.display.get_window_size()[0], 300 / 1080 * pygame.display.get_window_size()[1])
        self.image = pygame.Surface(self.size)
        self.value_color = color_black
        self.image.fill(color_white)
        self.rect = self.image.get_rect()
        self.rect.topleft = (pos.x / 1920 * pygame.display.get_window_size()[0], pos.y / 1080 * pygame.display.get_window_size()[1])
        self.button_text_font = pygame.font.SysFont('Times New Roman', int(20 / 1920 * pygame.display.get_window_size()[0]))
        self.min_value = min_value
        self.max_value = max_value
        self.current_value = 0
        self.current_value_diagram_size = 0
        self.next_value = 0
        self.value_change = 0
        self.preview_mode = False
        self.done = False
    ...
    def update_preview(self):
        self.value_change = self.next_value - self.current_value
        if self.done:
            color = color_orange
        else:
            color = color_green
    ...
    def draw(self):
        ...
        if self.diagram_unlocked:
            diagram_height = self.rect.size[1]

            # diagram boarder
            pygame.draw.rect(self.image, color_red, pygame.Rect(0, 0, self.rect.size[0], diagram_height), 4)
            ...
            if self.min_value < 0:
                self.current_value_diagram_size = diagram_height / (self.max_value + - self.min_value) * (
                        self.current_value - self.min_value)
            else:
                self.current_value_diagram_size = diagram_height / self.max_value * self.current_value
            # diagram bar
            pygame.draw.rect(self.image, self.value_color,
                             pygame.Rect(4, self.rect.size[1] - self.current_value_diagram_size + 3,
                                         self.rect.size[0] - 8,
                                         self.current_value_diagram_size - 6))
...
class HelpScreen(pygame.sprite.Sprite):
    def __init__(self, camera):
        ...
        self.special_case_picture = pygame.transform.scale(self.special_case_picture, (400 * self.scale_x, 400 * self.scale_y))
        ...
        if self.active:
            ...
            if self.help_step == 1:
                ...
                draw_text(Vector2(225 * x, 60 * y), self.image, small_font_size,
                          "Mit dem Kopf Spielanleitung öffnet sich die originale\n"
                ...
                draw_text(Vector2(1095 * x, 50 * y), self.image, small_font_size,
                          ...
                          "würden.\n"
                          "Gekennzeichnet wird dies mithilfe einer Zahl über beziehungsweise\n"
                          "bei negativen Zahlen unter dem aktuellen Wert!\n\n"
                          "Hinweis:"
                ...
                draw_text(Vector2(595 * x, 460 * y), self.image, small_font_size,
                          "Der Knopf Bester Zug verteilt die verfügbaren Aktionspunkte so gut\n"
                          "es geht sinnvoll auf die Bereiche.\n"
                          "Dahinter steckt ein Agent(KI), der darauf trainiert wurde.\n\n"
                          "Achtung!\n"
                          "Die Anzahl ist pro Spiel auf 5 begrenzt!")
